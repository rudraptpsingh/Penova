        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: DraftrSpace.l) {
                    ...
                    DraftrTextField(label: "Name", text: $name, placeholder: "Iqbal")
                    chipRow("Role", selection: $role, options: CharacterRole.allCases) { $0.display }
                    DraftrTextField(label: "Age", text: $ageText, placeholder: "mid-40s or 42")
                    DraftrTextField(label: "Occupation", text: $occupation, placeholder: "Night porter")
                    DraftrTextField(label: "Goal", text: $goal, placeholder: "What they want.")
                    DraftrTextField(label: "Conflict", text: $conflict, placeholder: "What's in their way.")
                    DraftrTextField(label: "Traits", text: $traitsRaw, placeholder: "patient, observant, quiet")
                    DraftrTextField(label: "Notes", text: $notes, placeholder: "Anything else worth remembering.")
                    DraftrButton(title: editing == nil ? "Create character" : "Save changes", variant: .primary) { save() }
                        .disabled(!canSave)
        ...
        VStack(alignment: .leading, spacing: DraftrSpace.s) {
            ...
            FlowLayout(spacing: DraftrSpace.s) {
                ForEach(projects) { p in
                    DraftrChip(text: p.title, isSelected: p.id == selectedProjectId) {
        ...
        VStack(alignment: .leading, spacing: DraftrSpace.s) {
            ...
            FlowLayout(spacing: DraftrSpace.s) {
                ForEach(options, id: \.self) { option in
                    DraftrChip(text: title(option), isSelected: selection.wrappedValue == option) {
