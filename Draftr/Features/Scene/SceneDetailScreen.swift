            ...
            .toolbar {
                ToolbarItem(placement: .topBarTrailing) {
                    Button {
                        ...
                        DraftrIconView(.bookmark, size: 18,
                                       color: scene.bookmarked ? DraftrColor.amber : DraftrColor.snow3)
                    }
                }
            }

            if !scene.elements.isEmpty {
                DraftrFAB(icon: .plus) { showElementType = true }
                    .padding(DraftrSpace.l)
            }
        }
    ...
    private var content: some View {
        ...
        case .action:
            Text(element.text.isEmpty ? "Action…" : element.text)
                .font(DraftrFont.monoScript)
                .foregroundStyle(element.text.isEmpty ? DraftrColor.snow4 : DraftrColor.snow)
