...
| Inter / Roboto Mono fonts | Register via `Info.plist` `UIAppFonts`, expose through `PenovaFont.*` |
| 8pt spacing ladder | `PenovaSpace.*` (xs=4, s=8, sm=12, m=16, l=24, xl=40, xxl=64) |
| Radius 8 / 12 / ∞ | `PenovaRadius.sm` / `.md` / `.full` via `.clipShape(RoundedRectangle(cornerRadius:))` or `.clipShape(Capsule())` |
| Motion 120 / 200 / 320 ms | `PenovaMotion.fast` / `.base` / `.slow` using `Animation.timingCurve(0.2, 0.8, 0.2, 1, duration:)` |
...
| Tag pills | `PenovaTag` (`RoundedRectangle(cornerRadius: PenovaRadius.sm)` filled with `PenovaColor.ink3`) |
