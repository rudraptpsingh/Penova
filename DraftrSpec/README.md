...
Everything needed to build **Draftr v0.1** — a mobile-first screenwriting app for Indian screenwriters — as a native **Swift / SwiftUI** iOS app.
...
| [Icons.swift](Icons.swift) | `DraftrIcon` enum + `DraftrIconView` (prefers SF Symbols, falls back to bundled SVG) |
...
| [Components.swift](Components.swift) | `DraftrButton`, `DraftrTag`, `DraftrChip`, `CuePill`, `ProjectCard`, `SceneItem`, `CharacterCard`, `EmptyState`, `DraftrFAB`, `DraftrTextField`, `DraftrSectionHeader` |
...
2. Drop `DraftrSpec/` into the project (keep the folder).
3. Register Inter + Roboto Mono in `Info.plist` via `UIAppFonts`.
4. Add SVGs from `icons/` to the asset catalog as **Single Scale Vector** + **Preserve Vector Data** + **Template Image** render mode (name them to match `DraftrIcon.rawValue`).
...
- Figma: https://www.figma.com/design/bdPZlVwCRvNrVbVQgzqPFv/Draftr-iOS-App-Design
- PDF: `Draftr — iOS App Design.pdf` (in the repo root)
