...
## S01 Splash
- **Layout**: Centred `Draftr` wordmark in `DraftrFont.hero` on ink bg.
...
- **Detent**: `.large`.
- **Primitives**: `Speech` + `AVAudioEngine`; pulsing circle via `.scaleEffect` animated with `DraftrMotion.slow`.
