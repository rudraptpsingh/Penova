        ...
        VStack(alignment: .leading, spacing: PenovaSpace.s) {
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: PenovaSpace.s) {
                    PenovaChip(text: "Bookmarked", isSelected: bookmarkedOnly) {
                        bookmarkedOnly.toggle()
                    }
                    ForEach(SceneLocation.allCases, id: \.rawValue) { loc in
                        PenovaChip(text: loc.rawValue, isSelected: locationFilter == loc) {
                            locationFilter = locationFilter == loc ? nil : loc
                        }
                    }
                    ForEach(SceneTimeOfDay.allCases, id: \.rawValue) { time in
                        PenovaChip(text: time.rawValue, isSelected: timeFilter == time) {
