...
public enum PDFScreenplayParser {
    ...
    public static func parse(_ source: PDFLineSource) -> Result {
        ...
        // Collect every line on every page.
        var allLines: [PDFLine] = []
        for p in 0..<source.pageCount {
            allLines.append(contentsOf: source.lines(onPage: p))
        }
        ...
        // X-cluster the body to find role columns.
        let columns = inferColumns(body)
        ...
        for line in body {
            ...
            case .heading:
                ...
                currentScene = .init(heading: line.text.trimmingCharacters(in: .whitespaces),
                                     elements: [])
    ...
    private static func extractTitlePage(from lines: [PDFLine]) -> [String: String]? {
        ...
        // Fallback: first non-empty line = title; second-from-top
        // (skipping a blank) = "by Author"; lines toward the bottom-left
        // = contact block.
        let sorted = lines.sorted { $0.yTop > $1.yTop }
        let topish = sorted.prefix(8).map { $0.text.trimmingCharacters(in: .whitespaces) }
        guard let title = topish.first(where: { !$0.isEmpty }) else { return nil }
        fields["title"] = title

        if let byLine = topish.dropFirst().first(where: {
            $0.lowercased().hasPrefix("by ") || $0.lowercased().hasPrefix("written by")
        }) {
            let cleaned = byLine
    ...
    /// Some FDX PDFs print scene numbers in both margins.
    static func stripSceneNumberPrefix(_ text: String) -> String {
        var out = text
        // Leading "12   " or "A12  " or "12A  ".
        out = out.replacingOccurrences(
            of: #"^[A-Z]?[0-9]{1,4}[A-Z]?\s{2,}"#,
            with: "",
            options: .regularExpression
        )
        // Trailing "   12" or "   A12".
        out = out.replacingOccurrences(
            of: #"\s{2,}[A-Z]?[0-9]{1,4}[A-Z]?$"#,
            with: "",
            options: .regularExpression
        )
        return out
